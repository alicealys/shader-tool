...
namespace alys::shader
{
	...
	shader_object shader_object::parse(const std::string& data)
	{
		...
		for (auto i = 0u; i < header->chunk_count; i++)
		{
			...
			case chunk_shdr:
				...
				break;
			case chunk_rdef:
				obj.parse_resource_def_chunk(input_buffer, chunk_size);
				break;
			default:
	...
	{
		buffer.write("// Name                 Index   Mask Register SysValue  Format   Used\n");
		buffer.write("// -------------------- ----- ------ -------- -------- ------- ------\n");
		for (const auto& element : signature)
		{
			...
			buffer.write("// %-20s %5i %6s %8i %8s %7s %6s\n", element.name.data(),
	...
	void shader_object::dump_resource_definition(utils::string_writer& buffer, const resource_definition& resource_definition)
	{
		buffer.write("// %s\n", resource_definition.creator.data());
		buffer.write("//  \n");
		buffer.write("// constant buffers: \n");
		buffer.write("// \n");

		for (const auto& cb : resource_definition.constant_buffers)
		{
			buffer.write("// cbuffer %s\n", cb.name.data());
			buffer.write("// {\n");

			const auto get_var_type_name = [&](const std::uint32_t type)
			{
				switch (type)
				{
				case 0:
			...
			const auto format_var_type = [](const char* type, const std::uint32_t size)
				-> std::string
			{
				if (size == 1)
				{
			...
			for (const auto& var : cb.vars)
			{
				...
				const auto type_name = get_var_type_name(var.type.type);
				const auto type_fmt = format_var_type(type_name, var.type.col_count);

				if (var.type.element_count == 0)
				{
					var_fmt = std::format("{} {};", type_fmt, var.name.data());
				}
				else
				{
					var_fmt = std::format("{} {}[{}];", type_fmt, var.name.data(), var.type.element_count);
				}

				buffer.write("//     %-30s", var_fmt.data());
				buffer.write("// offset: % 5i size % 5i %s\n", var.offset, var.size, (var.flags & 2) != 0 ? "" : "[unused]");
			}
		...
		buffer.write("// resource bindings: \n");
		buffer.write("// \n");
		buffer.write("// name                           type    return type dimension sample count bind point bind count flags\n");
		buffer.write("// ------------------------------ ------- ----------- --------- ------------ ---------- ---------- -----\n");
		for (const auto& rb : resource_definition.resource_bindings)
		{
			buffer.write("// %-30s %-7i %-11i %-9i %-12i %-10i %-10i %-5i\n", rb.name.data(), 
				rb.shader_input_type,
				rb.resource_return_type,
				rb.resource_view_dim,
				rb.sample_count,
				rb.bind_point,
				rb.bind_count,
				rb.shader_input_flags
			);
		}
	...
	{
		buffer.write("// \n");
		buffer.write("// Generated by https://github.com/alicealys/shader-tool\n");
		buffer.write("// \n");
		buffer.write("// \n");

		if (this->resource_definition_.major_version != 0)
		{
			this->dump_resource_definition(buffer, this->resource_definition_);
		}
	...
	assembler shader_object::get_assembler()
	{
...
#pragma warning(pop)
	...
	void shader_object::parse_resource_def_chunk(utils::bit_buffer_le& chunk, const std::uint32_t size)
	{
		chunk.read_bytes(4); // "RDEF"
		chunk.read_bytes(4); /* len */

		const auto begin = chunk.total() / 8;

		const auto read_string = [&](const std::uint32_t offset)
		{
			std::string str;
			const auto start = chunk.total();
			chunk.set_byte(begin + offset);

			for (; auto c = chunk.read_bytes<char>(1); c != 0)
			{
				str.push_back(c);
			}

			chunk.set_byte(start / 8);
			return str;
		};

		const auto read_var_type = [&](const std::uint32_t offset)
		{
			const auto start = chunk.total();
			chunk.set_byte(begin + offset);

			resource_definition::constant_buffer::variable_type type{};
			type.class_ = chunk.read_bytes(2);
			type.type = chunk.read_bytes(2);
			type.row_count = chunk.read_bytes(2);
			type.col_count = chunk.read_bytes(2);
			type.element_count = chunk.read_bytes(2);
			type.member_count = chunk.read_bytes(2);
			type.member_offset = chunk.read_bytes(4);
			chunk.set_byte(start / 8);
			return type;
		};

		const auto cb_count = chunk.read_bytes(4);
		const auto cb_offset = chunk.read_bytes(4);

		const auto rb_count = chunk.read_bytes(4);
		const auto rb_offset = chunk.read_bytes(4);

		this->resource_definition_.minor_version = chunk.read_bytes(1);
		this->resource_definition_.major_version = chunk.read_bytes(1);
		this->resource_definition_.program_type = chunk.read_bytes(2);
		this->resource_definition_.flags = chunk.read_bytes(4);
		this->resource_definition_.creator = read_string(chunk.read_bytes(4));

		for (auto i = 0u; i < cb_count; i++)
		{
			chunk.set_byte(begin + cb_offset + i * (6 * 4));
			resource_definition::constant_buffer cb{};

			cb.name = read_string(chunk.read_bytes(4));
			const auto var_count = chunk.read_bytes(4);
			const auto var_offset = chunk.read_bytes(4);
			cb.size = chunk.read_bytes(4);
			cb.flags = chunk.read_bytes(4);
			cb.type = chunk.read_bytes(4);

			const auto start = chunk.total();
			for (auto o = 0u; o < var_count; o++)
			{
				chunk.set_byte(begin + var_offset + o * (10 * 4));
				resource_definition::constant_buffer::variable var{};
				var.name = read_string(chunk.read_bytes(4));
				var.offset = chunk.read_bytes(4);
				var.size = chunk.read_bytes(4);
				var.flags = chunk.read_bytes(4);
				var.type = read_var_type(chunk.read_bytes(4));
				const auto default_value_offset = chunk.read_bytes(4);
				chunk.read_bytes(4); // 00000
				chunk.read_bytes(4); // FFFFF
				chunk.read_bytes(4); // 00000
				chunk.read_bytes(4); // FFFFF

				cb.vars.emplace_back(var);
			}

			this->resource_definition_.constant_buffers.emplace_back(cb);
		}

		for (auto i = 0u; i < rb_count; i++)
		{
			chunk.set_byte(begin + rb_offset + i * (10 * 4));
			resource_definition::resource_binding rb{};
			rb.name = read_string(chunk.read_bytes(4));
			rb.shader_input_type = chunk.read_bytes(4);
			rb.resource_return_type = chunk.read_bytes(4);
			rb.resource_view_dim = chunk.read_bytes(4);
			rb.sample_count = chunk.read_bytes(4);
			rb.bind_point = chunk.read_bytes(4);
			rb.bind_count = chunk.read_bytes(4);
			rb.shader_input_flags = chunk.read_bytes(4);
			chunk.read_bytes(4); // 0000
			chunk.read_bytes(4); // 0000
			this->resource_definition_.resource_bindings.emplace_back(rb);
		}
	}
