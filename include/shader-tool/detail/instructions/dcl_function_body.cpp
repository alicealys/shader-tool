...
{
	instruction_t dcl_function_body::read(utils::bit_buffer_le& input_buffer, const std::uint32_t version)
	{
	...
	void dcl_function_body::write(utils::bit_buffer_le& output_buffer, const instruction_t& instruction, const std::uint32_t version)
	{
	...
	void dcl_function_body::dump(utils::string_writer& buffer, const instruction_t& instruction, const std::uint32_t version)
	{
		dump_opcode(buffer, instruction.opcode);
		buffer.write(" fb%i", instruction.operands[0].custom.u.value);
	}

	std::uint32_t dcl_function_body::get_flags()
	{
