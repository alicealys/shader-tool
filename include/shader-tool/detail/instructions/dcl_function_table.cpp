...
{
	instruction_t dcl_function_table::read(utils::bit_buffer_le& input_buffer, const std::uint32_t version)
	{
	...
	void dcl_function_table::write(utils::bit_buffer_le& output_buffer, const instruction_t& instruction, const std::uint32_t version)
	{
	...
	void dcl_function_table::dump(utils::string_writer& buffer, const instruction_t& instruction, const std::uint32_t version)
	{
		assert(instruction.operands.size() >= 2 && instruction.operands.size() == (2 + instruction.operands[1].custom.u.value));
	...
	std::uint32_t dcl_function_table::get_flags()
	{
