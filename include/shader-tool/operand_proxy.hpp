...
namespace alys::shader
{
	...
	public:
		...
		public:
			with_components()
				: current_(std::make_shared<detail::operand_t>())
			{
			}

			with_components(const detail::operand_t& operand)
			...
			void set(const detail::operand_t& operand, const std::uint32_t x, const std::uint32_t y = component_none,
				const std::uint32_t z = component_none, const std::uint32_t w = component_none);

		private:
			bool has_set_components_{};
			std::uint8_t components_[4]{};
			std::shared_ptr<detail::operand_t> current_{};

		};

		operand_proxy(const detail::operand_t& operand);
